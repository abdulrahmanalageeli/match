...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      // Get unread inbound WhatsApp message count + recent messages summary
      if (action === "get-whatsapp-unread") {
        try {
          const { data: messages, error } = await supabase
            .from("whatsapp_messages")
            .select("id, assigned_number, phone_number, message_body, button_payload, button_text, needs_organizer, created_at")
            .eq("direction", "inbound")
          ...
          if (error) {
            console.error("get-whatsapp-unread error:", error)
            return res.status(200).json({ success: true, count: 0, messages: [], needs_organizer: 0 })
          }

          // Count messages flagged as needing organizer attention
          const needsOrganizer = (messages || []).filter(m => m.needs_organizer).length
          ...
          console.error("get-whatsapp-unread exception:", err)
          return res.status(200).json({ success: true, count: 0, messages: [], needs_organizer: 0 })
        }
      }

      // Mark WhatsApp messages as seen by admin
      if (action === "mark-whatsapp-seen") {
        try {
          const { error } = await supabase
            .from("whatsapp_messages")
          ...
          if (error) {
            console.error("mark-whatsapp-seen error:", error)
            return res.status(500).json({ error: error.message })
          }

          return res.status(200).json({ success: true })
        } catch (err) {
          console.error("mark-whatsapp-seen exception:", err)
