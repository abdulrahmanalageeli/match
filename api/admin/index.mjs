...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "add-manual-match") {
      try {
        const { participant1, participant2, eventId } = req.body

        if (!participant1 || !participant2 || !eventId) {
          return res.status(400).json({ error: "Missing required fields: participant1, participant2, eventId" })
        }

        if (participant1 === participant2) {
          return res.status(400).json({ error: "Cannot match a participant with themselves" })
        }
        ...
        if (!participants || participants.length !== 2) {
          return res.status(400).json({ error: "One or both participants not found" })
        }
        ...
        const calculateMBTICompatibility = (type1, type2) => {
          ...
          if (!type1 || !type2 || !MBTI_COMPATIBILITY[type1]) return 0
          
          const compatibility = MBTI_COMPATIBILITY[type1]
          if (compatibility.top1 === type2) return 10
          if (compatibility.top2 === type2) return 8
          if (compatibility.top3 === type2) return 6
          if (compatibility.bonus.includes(type2)) return 4
          return 2
        }

        const calculateAttachmentCompatibility = (style1, style2) => {
          const ATTACHMENT_COMPATIBILITY = {
            'secure': { 'secure': 10, 'anxious': 8, 'avoidant': 6, 'disorganized': 4 },
            'anxious': { 'secure': 8, 'anxious': 4, 'avoidant': 2, 'disorganized': 3 },
            'avoidant': { 'secure': 6, 'anxious': 2, 'avoidant': 5, 'disorganized': 3 },
            'disorganized': { 'secure': 4, 'anxious': 3, 'avoidant': 3, 'disorganized': 2 }
          }
          return ATTACHMENT_COMPATIBILITY[style1]?.[style2] || 0
        }

        const calculateCommunicationCompatibility = (style1, style2) => {
          const COMMUNICATION_COMPATIBILITY = {
            'direct': { 'direct': 8, 'indirect': 6, 'assertive': 9, 'passive': 4 },
            'indirect': { 'direct': 6, 'indirect': 7, 'assertive': 5, 'passive': 8 },
            'assertive': { 'direct': 9, 'indirect': 5, 'assertive': 7, 'passive': 3 },
            'passive': { 'direct': 4, 'indirect': 8, 'assertive': 3, 'passive': 6 }
          }
          return COMMUNICATION_COMPATIBILITY[style1]?.[style2] || 0
        }
        ...
        // Calculate lifestyle compatibility (age difference)
        const ageDiff = Math.abs((p1.age || 25) - (p2.age || 25))
        const lifestyleScore = Math.max(0, 10 - ageDiff)

        // Calculate core values compatibility (based on survey data similarity)
        const coreValuesScore = 7 // Default reasonable score

        // Calculate vibe compatibility (default high for manual matches)
        const vibeScore = 8 // Assume good vibe for manual matches
        ...
        console.log(`✅ Manual match created: #${participant1} ↔ #${participant2} (Score: ${totalScore}%)`)

        return res.status(200).json({ 
          success: true, 
          message: `Manual match created successfully: #${participant1} ↔ #${participant2}`,
          match: insertData[0],
          compatibility_score: totalScore
        })

      } catch (error) {
        console.error("Error in add-manual-match:", error)
        return res.status(500).json({ error: "Failed to create manual match" })
      }
    }
