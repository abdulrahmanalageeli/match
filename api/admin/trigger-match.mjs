...
async function calculateCombinedVibeCompatibility(profileA, profileB) {
  try {
    const systemMessage = `أنت محلل توافق شخصي. قيّم توافق شخصين وفق قواعد صريحة وعددية فقط. لا تكن متساهلاً أو متشائماً؛ كن محايداً ومنضبطاً بالقواعد.

احسب مجموع نقاط من 0 إلى 15 وفق المحاور الآتية (أقصى 15):
1) نهاية الأسبوع (0-3)
2) الهوايات/الاهتمامات (0-4)
3) الذوق الموسيقي/الفني (0-3)
4) نمط المحادثة (عميق/خفيف) (0-2)
5) كيف يصفهم الأصدقاء (0-1)
6) كيف يصفون أصدقاءهم (0-1)
7) مكافأة/خصم التوازن (−2 إلى +1) لوجود تكامل مفيد أو تعارض صارخ.

قواعد المنح التفصيلية:
• نهاية الأسبوع: تطابق واضح = +3، تقارب جزئي (بيت/خروج معتدل) = +2، تباين كبير (بيت دائماً مقابل خروج دائماً) = +0.
• الهوايات: 2+ هوايات مشتركة مباشرة = +4، تقاطع روحي (مثلاً قراءة/أفلام = محتوى) = +2، لا تقاطع = +0.
• الموسيقى/الفن: نفس العائلة (عربي مع عربي أو غربي مع غربي) = +3، أذواق مختلفة لكن قابلة للتعايش = +1، تعارض حاد (طرف يكره فئة الآخر) = +0.
• نمط المحادثة: نفس التفضيل = +2، اختلاف بسيط = +1، اختلاف حاد = +0.
• وصف الأصدقاء لهم: تطابق في السمات الاجتماعية/الانطوائية/الجدية = +1، وإلا 0.
• كيف يصفون أصدقاءهم: تطابق القيم (ولاء، دعم، مرح) = +1، وإلا 0.
• مكافأة/خصم التوازن: تكامل نافع واضح (هادئ + اجتماعي، منظم + عفوي) = +1. تعارض صارخ في محورين أو أكثر (أسلوب Weekend + محادثة مثلاً) = −2.
