    ...
    try {
      ...
      try {
        ...
        const { data: e3State } = await supabase
          .from("event_state")
          .select("phase,current_event_id,results_visible")
        ...
        if (e3Finished) {
          ...
          try {
            const { data, error } = await supabase
              .from("event3_matches")
              .select("event_id,phase2_partner,phase2_score,phase2_word,phase2_feedback,phase3_partner,phase3_score,phase3_word,phase3_feedback,match_preference")
              .eq("match_id", E3_MATCH_ID)
              .eq("participant_number", participant.assigned_number)
            e3Matches = data
            e3MatchErr = error
        ...
        if (e3Matches && e3Matches.length > 0) {
          ...
          // Helper to get compatibility breakdown from cache (event-independent)
...
معلومات ${firstName} (لا تذكرها صراحة إطلاقاً — وظّفها بذكاء):
...
قاعدة إلزامية صفر — تعريب الاسم:
لو كان "${firstName}" مكتوب بحروف لاتينية، حوّله لكتابة عربية طبيعية تناسب النطق السعودي (bayan ← بيان، Thamer ← ثامر، Reem ← ريم). لا تترك أي اسم بحروف إنجليزية داخل رسالة عربية.

قاعدة إلزامية أولى — مطابقة الجنس:
كل فعل وصفة وضمير يجب أن يطابق ${gender === "male" ? "ذكر" : gender === "female" ? "أنثى" : "الجنس"} تماماً. لا استثناء — هذا أهم من أي قاعدة أسلوبية أخرى.

قاعدة إلزامية ثانية — التركيز لا الحشو:
اختر عنصرين فقط من المعلومات الست — الأكثر تفرداً أو تناقضاً — وابنِ الرسالة حولهما فقط. تجاهل الباقي.

قاعدة إلزامية ثالثة — تنويع مجال الاستعارة:
لا تلجأ افتراضياً لكلمات "هدوء/لحن/نغمة/إيقاع" إلا لو فيه ارتباط صريح بالموسيقى. اختر المجال المناسب من: حركة/رياضة، حرفة يدوية، طبيعة، بناء/هندسة، طبخ، كتابة/قراءة، أو غيره حسب بيانات الشخص الفعلية.

قاعدة إلزامية رابعة — نكتة كسر جليد (اختيارية بس مرغوبة):
لو المساحة تسمح، أضف جملة خفيفة الظل عن موقف الفعالية نفسه (التعارف "الأعمى"، أول لقاء، عدم معرفة أحد) — مو عن شخصية المستلم. خفيفة، ذكية، جملة وحدة، بدون تورية مجهدة أو "هاها" مفتعلة. اختبار: لو قريتها لصديق بصوت عالي، هل يبتسم طبيعي؟ لو لأ، احذفها.

كيف توظّف المعلومات:
1. حوّل المعلومة إلى استعارة من مجالها الصحيح — لا تذكرها كحقيقة مباشرة
2. لو دمجت عنصرين، ادمجهم بجملة وحدة غير متوقعة
3. اجعل الإطراء يجي من زاوية غير مباشرة — كأن شخص ثاني بيلاحظ فيه الشي

فكّر بصمت قبل الكتابة (لا تُخرج هذا التفكير):
- ما العنصرين الأكثر تفرداً؟
- من أي مجال تجي الاستعارة المناسبة؟
- فيه مكان لنكتة خفيفة عن الموقف بدون ما تكسر تدفق الرسالة؟
- وش الجملة الختامية اللي تخلي الشخص يحس "كيف عرفوا هذا عني؟"

بنية الرسالة:
1. ابدأ باسم "${firstName}" (بصيغته العربية) بطريقة طبيعية
2. جملة أو جملتين توظّف العنصرين المختارين عبر استعارة من مجالها الصحيح
3. (اختياري) جملة خفيفة كسر جليد عن موقف الفعالية
4. جهّزه للفعالية بإشارة لتجربة تعارف ذكية ونقاشات — بدون شرح آلية المطابقة
5. اختم بجملة "إصابة" — لا سؤال افتراضياً. جملة تسمّي أو تؤكد شيئاً حقيقياً عنه. لو حسيت إن سؤال يخدم الرسالة (نادراً)، خله واقعي بسيط يُجاب بجملة وحدة — ممنوع الأسئلة الشاعرية المجردة.

ممنوعات صارمة:
- ممنوع سرد أي معلومة بشكل مباشر أو حرفي
- ممنوع كلام عن الحب أو العاطفة أو الزواج
- ممنوع عبارات مستهلكة مثل "أهلاً وسهلاً"
- ممنوع ذكر أنك AI أو تفاصيل تقنية عن آلية المطابقة
- ممنوع المواعظ أو النصائح المباشرة
- ممنوع تجاوز 100 كلمة تحت أي ظرف — عُدّ الكلمات قبل الإخراج
- ممنوع أي مقدمة أو علامات اقتباس — أخرج نص الرسالة فقط

أمثلة للمعايرة (للفهم فقط):

سيئ: "يا أحمد، نعرف إنك تحب كرة القدم وتقضي الويكند مع الشباب، فرصتك الليلة تتعرف على ناس جدد."

سيئ (استعارة مكررة + نكتة متكلفة): "يا سارة، فيك هدوء يشبه لحن هادئ. استعدي لتوافق ما بعده أعمى ولا بصير!"

جيد: "يا فهد، بين إتقانك لتفاصيل الشطرنج وحبك تصلح الأشياء بيدك، أنت من النوع اللي يشوف النقلة الرابعة قبل لا يحرك الأولى. وإذا حسيت بغرابة إنك ما تعرف حد هناك... زين، ولا أحد فيهم يعرفك، فالكل بنفس المركب. الليلة الطاولة اللي بتجلس فيها فيها ناس بيلاحظون هالشي فيك من أول خمس دقايق. وناس مثلك ما يحتاجون يثبتون شي — يكفي يحضرون."`
