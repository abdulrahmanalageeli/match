...
export default function ChoiceRewardsAdmin({ api, eventId, phase, testMode, disabled = false }: { api: (action: string, data?: any) => Promise<any>; eventId: number; phase: string; testMode: boolean; disabled?: boolean }) {
  ...
  return <details className="rounded-2xl border border-amber-400/20 bg-gradient-to-l from-amber-950/20 to-purple-950/20 p-4 text-right">
    <summary className="cursor-pointer text-sm font-bold text-amber-200"><Gift size={16} className="ml-2 inline"/> هدايا الأكثر اختياراً · خصم 50% {rewards.length > 0 && `(${rewards.length})`}</summary>
