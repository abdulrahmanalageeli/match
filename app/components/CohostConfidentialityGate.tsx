...
type CohostIdentity = { number: 1372 | 1470; displayName: string; profileName: string }
type AgreementResponse = { agreement: typeof COHOST_AGREEMENT; agreement_hash: string; accepted: boolean; accepted_at: string | null; cohost: CohostIdentity }
...
export default function CohostConfidentialityGate({ token, onAccepted, onLogout }: {
  token: string
  onAccepted: (token: string, identity: CohostIdentity) => void
  onLogout: (message?: string, clearAccount?: boolean) => void
}) {
  ...
  useEffect(() => {
    ...
    agreementRequest<AgreementResponse>(token, "e3-cohost-agreement", controller.signal).then(result => {
      ...
      if (!result.cohost?.number || !result.cohost?.displayName) throw new Error("تعذر التحقق من حساب المضيف المساعد")
      if (result.accepted) { onAccepted(token, result.cohost); return }
      ...
      if (controller.signal.aborted) return
      if (requestError.status === 401 || requestError.status === 403) { onLogout("انتهت الجلسة. يرجى تسجيل الدخول مرة أخرى"); return }
      setError(requestError.status === 423 ? "أوقف المنظم الوصول مؤقتاً. أعد المحاولة بعد فتح اللوحة" : "تعذر تحميل التعهد. تحقق من الاتصال ثم أعد المحاولة")
  ...
  const accept = async (event: FormEvent) => {
    ...
    try {
      ...
      onAccepted(result.token, data.cohost)
    } catch (requestError) {
      if (controller.signal.aborted) return
      const failure = requestError as AgreementError
      if (failure.status === 401 || failure.status === 403) { onLogout("انتهت الجلسة. يرجى تسجيل الدخول مرة أخرى"); return }
      if (failure.code === "AGREEMENT_CHANGED") { setRetry(value => value + 1); return }
      setError(failure.status === 423 ? "أوقف المنظم الوصول مؤقتاً. لا يمكن إكمال الدخول الآن" : failure.name === "TimeoutError" || failure.name === "AbortError" ? "انتهت مهلة الاتصال. أعد المحاولة؛ لن تتكرر الموافقة المسجلة" : failure.message || "تعذر حفظ الموافقة. أعد المحاولة")
    } finally {
    ...
    <main className="min-h-[100dvh] bg-[#06090f] px-4 py-6 text-white sm:py-10" dir="rtl">
      <div className="mx-auto w-full max-w-2xl">
        <header className="mb-6 flex items-start gap-3">
          ...
          <div><p className="text-xs font-bold text-teal-300">التوافق الأعمى · لوحة المضيف المساعد</p><h1 className="mt-1 text-xl font-black sm:text-2xl">تعهد السرية وحماية البيانات</h1><p className="mt-2 text-sm leading-6 text-slate-400">قبل الاطلاع على بيانات المشاركين، يجب قراءة التعهد وتأكيد الموافقة عليه.</p></div>
        </header>
        ...
        {data ? <>
          ...
          <form onSubmit={accept} className="mt-4 rounded-3xl border border-teal-300/15 bg-teal-950/15 p-5 sm:p-6">
            <p className="text-xs font-bold text-slate-400">التعهد مرتبط بهذا الحساب</p>
            <div className="mt-2 rounded-xl border border-white/10 bg-black/20 px-4 py-3"><p className="font-black text-white">{data.cohost.profileName}</p><p className="mt-1 text-xs text-teal-200">{data.cohost.displayName} · #{data.cohost.number}</p></div>
            ...
            <button type="submit" disabled={saving || !consent} aria-busy={saving} className="mt-5 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-teal-300 px-3 font-black text-slate-950 disabled:opacity-40">{saving ? <Loader2 size={18} className="animate-spin" /> : <ShieldCheck size={18} />}{saving ? "جارٍ حفظ الموافقة…" : "أوافق وأدخل اللوحة"}</button>
          </form>
        ...
        <button type="button" onClick={() => onLogout("", true)} disabled={saving} className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl text-sm text-slate-400 disabled:opacity-40"><LogOut size={16} /> تغيير الحساب</button>
      </div>
    </main>
  )
}
