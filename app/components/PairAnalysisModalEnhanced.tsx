...
const LIFESTYLE_QUESTIONS: Record<number, string> = {
  ...
  3: "أي نمط تواصل يريحك أكثر خلال الأسبوع؟",
  4: "أي ترتيب يريحك أكثر لطلعة نهاية الأسبوع؟",
  5: "أي شكل لنهاية الأسبوع يعطيك طاقة أكثر؟",
}

const SYNERGY_QUESTIONS: Record<string, string> = {
  conversational_role: "في أي جلسة أو جمعة، وش يكون دورك العفوي؟",
  conversation_depth_pref: "أي شكل للسالفة يخليك تسترسل أكثر؟",
  social_battery: "بعد ساعة من السوالف مع ناس جدد، كيف تحس طاقتك؟",
  humor_subtype: "وش أكثر شيء يضحكك من قلب؟",
  curiosity_style: "في أول تعارف، أي نوع من التفاعل يشدك أكثر؟",
  silence_comfort: "إذا توقف الحوار لعدة ثوانٍ، وش تسوي غالبًا؟",
}
...
const ATTACHMENT_QUESTIONS: Record<string, { label: string, options: Record<string, string> }> = {
  attachment_1: { label: "بعد عدة أيام بدون تواصل:",
    options: { 'أ': 'أفترض أنه مشغول', 'ب': 'أتساءل إذا تغيّر شيء', 'ج': 'لا أبادر غالبًا', 'د': 'أرغب بالتواصل لكن أتردد' } },
  attachment_2: { label: "عند حدوث خلاف:",
    options: { 'أ': 'أواجه بهدوء', 'ب': 'أتجنب المواجهة', 'ج': 'أنسحب', 'د': 'أقترب ثم أبتعد' } },
  attachment_3: { label: "لما تصير العلاقة أقرب:",
    options: { 'أ': 'أشارك أكثر بالتدريج', 'ب': 'أحتاج وضوح الاهتمام', 'ج': 'أحتاج حدودًا ومساحة', 'د': 'أرغب بالقرب مع حذر' } },
  attachment_4: { label: "وقت الضغط أو المشكلة:",
    options: { 'أ': 'أشرح لشخص أثق فيه', 'ب': 'أتواصل أكثر وأحتاج ردًا سريعًا', 'ج': 'أرتب الموضوع بنفسي أولًا', 'د': 'أطلب الدعم ثم أبتعد قليلًا' } },
...
const COMMUNICATION_QUESTIONS: Record<string, { label: string, options: Record<string, string> }> = {
  communication_1: { label: "بعد تجاوز شخص لحدودك:",
    options: { 'أ': 'أقول مباشرة وش أزعجني', 'ب': 'ما أفتح الموضوع', 'ج': 'أعترض بنبرة قوية', 'د': 'يتغيّر تعاملي بدون كلام مباشر' } },
  communication_2: { label: "عند احتياج شيء مهم:",
    options: { 'أ': 'أقول طلبي بوضوح', 'ب': 'أنتظر أن يلاحظ', 'ج': 'أكرر أو أشدد على طلبي', 'د': 'أقول عادي ويظهر انزعاجي' } },
  communication_3: { label: "عند اختلاف رأي جماعي:",
    options: { 'أ': 'أشرح اختلافي', 'ب': 'أمشي مع رأي المجموعة', 'ج': 'أرد بقوة على نقاط الضعف', 'د': 'يظهر اعتراضي لاحقًا بشكل غير مباشر' } },
  communication_4: { label: "لما يرتفع توترك:",
    options: { 'أ': 'أشرح توتري', 'ب': 'أسكت أو أنسحب', 'ج': 'يصير كلامي أحدّ', 'د': 'يقل تواصلي أو يبرد' } },
  communication_5: { label: "عند اختلاف مهم مع شخص قريب:",
    options: { 'أ': 'أوضح موقفي وأفهمه', 'ب': 'أسكت حتى لا تصبح مشكلة', 'ج': 'أتمسك وأحاول إثبات رأيي', 'د': 'أوصل اعتراضي بالمزح أو التلميح' } },
}

const CORE_VALUES_QUESTIONS: Record<string, { label: string, options: Record<string, string> }> = {
  core_values_1: { label: "عند سؤال المدير عن خطأ صديق:",
    options: { 'أ': 'أقول اللي أعرفه', 'ب': 'أترك له شرح التفاصيل', 'ج': 'أحافظ على خصوصيته' } },
  core_values_2: { label: "ترك الوظيفة لمشروع مجرّب:",
    options: { 'أ': 'يبدأ بدوام كامل', 'ب': 'ينتقل تدريجيًا', 'ج': 'ينتظر دخلًا ثابتًا' } },
  core_values_3: { label: "الاختلاف في القيم الأساسية:",
    options: { 'أ': 'جزء طبيعي من العلاقة', 'ب': 'يحتاج حدودًا واضحة', 'ج': 'أفضل التقارب في الأساسيات' } },
  core_values_4: { label: "وقت صعب مع مساحة من القريب:",
    options: { 'أ': 'المساحة تناسبني', 'ب': 'أفضل رسالة أحيانًا', 'ج': 'أحتاج حضورًا متكررًا' } },
  core_values_5: { label: "خلاف صديق مع شخص تعرفه:",
    options: { 'أ': 'أفصل بين العلاقتين', 'ب': 'أفهم وأقلل مؤقتًا', 'ج': 'أنهي علاقتي بالشخص الآخر' } },
}
...
export default function PairAnalysisModal({ open, onOpenChange, a, b, pair }: PairAnalysisModalProps) {
  ...
  // Detailed synergy contributions (raw out of 30, plus scaled to 35)
  const computeSynergyDetails = (pa: any, pb: any) => {
    ...
    const a39 = String(getA(pa, 'curiosity_style') || '').toUpperCase()
    const b39 = String(getA(pb, 'curiosity_style') || '').toUpperCase()
    const a41v = String(getA(pa, 'silence_comfort') || '').toUpperCase()
    const b41v = String(getA(pb, 'silence_comfort') || '').toUpperCase()
    ...
    let q41 = 0
    if (a41v && b41v) {
      if ((a41v === 'A' && b41v === 'B') || (a41v === 'B' && b41v === 'A')) q41 = 5
      else if (a41v === 'A' && b41v === 'A') q41 = 3
      else if (a41v === 'B' && b41v === 'B') q41 = 0
    }
  ...
  // Humor multiplier like backend (1.15 full, 1.05 partial, 1.0 none)
  const computeHumorMultiplier = (pa: any, pb: any) => {
    const getAns = (p: any, key: string) => p?.survey_data?.answers?.[key] ?? p?.survey_data?.[key] ?? p?.[key] ?? undefined
    const hA = getAns(pa, 'humor_banter_style')
    const hB = getAns(pb, 'humor_banter_style')
    const oA = getAns(pa, 'early_openness_comfort')
    const oB = getAns(pb, 'early_openness_comfort')
  ...
  // Humor multiplier + synergy details + AI
  const humorMultiplier = useMemo(() => computeHumorMultiplier(a, b), [a, b])
  ...
  const handleCopy = async () => {
    try {
      const text = pair?.reason
        ? `السبب: ${pair.reason}\nالتفاعل: ${scores.synergy.toFixed(1)}/30، الطاقة: ${scores.vibe.toFixed(1)}/25، نمط الحياة: ${scores.lifestyle.toFixed(1)}/10، الدعابة/الانفتاح: ${scores.humor.toFixed(1)}/15، التواصل: ${scores.communication.toFixed(1)}/3، الأهداف/القيم: ${scores.intent.toFixed(1)}/5`
        : `التفاعل: ${scores.synergy.toFixed(1)}/30، الطاقة: ${scores.vibe.toFixed(1)}/25، نمط الحياة: ${scores.lifestyle.toFixed(1)}/10، الدعابة/الانفتاح: ${scores.humor.toFixed(1)}/15، التواصل: ${scores.communication.toFixed(1)}/3، الأهداف/القيم: ${scores.intent.toFixed(1)}/5`
        ...
        <div className="flex flex-col gap-4 px-6 py-5 border-b border-white/10 bg-linear-to-l from-slate-900 via-slate-800 to-slate-900">
          ...
          <div className="flex items-center justify-between gap-3 text-xs text-slate-300">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-white/5 border border-white/15">
                  ...
                  <div className="bg-white/10 border border-white/20 rounded-xl p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      {pair?.humor_early_openness_bonus && pair.humor_early_openness_bonus !== 'none' && (
                        <div className="flex items-center gap-2 text-amber-300" title="تطابق الدعابة وال/or الانفتاح المبكر يمنح مضاعفًا للنتيجة النهائية">
                      ...
                      {pair?.intent_boost_applied && (
                        <div className="flex items-center gap-2 text-emerald-300" title="كلا الطرفين اختار الهدف B ⇒ ×1.1">
                          <BadgeCheck className="w-4 h-4" /> مضاعف الهدف ×1.1
                        </div>
                      )}
                      {pair?.attachment_penalty_applied && (
                        <div className="flex items-center gap-2 text-red-300" title="قلق × تجنّب ⇒ −5 قبل تطبيق القيود">
                      ...
                      {pair?.humor_clash_veto_applied && (
                        <div className="flex items-center gap-2 text-red-300" title="تعارض قوي في الدعابة (A↔D) ⇒ سقف 50%">
                          <Info className="w-4 h-4" /> تعارض الدعابة: سقف 50%
                        </div>
                      )}
                      {pair?.cap_applied != null && (
                        <div className="flex items-center gap-2 text-yellow-300" title="تم تطبيق سقف نهائي على النتيجة">
            ...
            <TabsContent value="humor">
              <div className="bg-white/10 border border-white/20 rounded-xl p-4 mt-3">
                ...
                <div className="mb-3 text-xs">
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md border ${humorMultiplier > 1 ? 'bg-emerald-500/10 text-emerald-300 border-emerald-400/30' : 'bg-white/5 text-slate-300 border-white/20'}`} title="مضاعف مختلط من تطابق الدعابة والانفتاح">
                    مضاعف: ×{humorMultiplier.toFixed(2)}
                  </span>
                </div>
                {(() => {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div className="bg-slate-900/70 border border-white/20 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {aAI && (
                          <button onClick={async () => { try { await navigator.clipboard.writeText(aAI); } catch {} }} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-white/10 hover:bg-white/20 border border-white/15 text-slate-200 text-xs">
                    ...
                    <div className="bg-slate-900/70 border border-white/20 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {bAI && (
                          <button onClick={async () => { try { await navigator.clipboard.writeText(bAI); } catch {} }} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-white/10 hover:bg-white/20 border border-white/15 text-slate-200 text-xs">
                            <Copy className="w-3.5 h-3.5" /> نسخ
                          </button>
                        )}
                      </div>
