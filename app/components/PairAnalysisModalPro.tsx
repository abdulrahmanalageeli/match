...
const SYNERGY_OPTIONS: Record<string, Record<string, string>> = {
  ...
  conversation_depth_pref: {
    A: "أ. نأخذ موضوعًا واحدًا ونناقش أفكاره وأسبابه من أكثر من زاوية.",
    B: "ب. نتبادل عدة قصص وأحداث وتجارب واقعية وننتقل بينها.",
  },
  social_battery: {
  ...
  curiosity_style: {
    A: "أ. الطرف الآخر يسألني عن دوافع وتجارب شخصية وأشارك إجاباتي.",
    B: "ب. أنا أسأل وأستكشف تفاصيل وقصص حياة الطرف الآخر.",
    C: "ج. نتبادل تعليقات قصيرة ومزحًا سريعًا أكثر من الأسئلة الطويلة.",
  },
  silence_comfort: {
    A: "أ. أفتح موضوعًا أو أسأل سؤالًا جديدًا بسرعة.",
    B: "ب. أترك الهدوء قليلًا وأنتظر أشوف كيف تكمل السالفة.",
  },
}
...
const LIFESTYLE_OPTIONS: Record<number, Record<string, string>> = {
  ...
  3: {
    'أ': "أ. أحب يكون عندي وقت منفصل لنفسي بشكل شبه يومي",
    'ب': "ب. أحب أيامًا فيها تواصل ووقت مشترك، وأيامًا أهدأ فيها لوحدي",
    'ج': "ج. أحب التواصل المتكرر ومشاركة أغلب الأنشطة والأحداث اليومية",
  },
  4: {
    'أ': "أ. نتفق من بدري على الوقت والمكان والخطة",
    'ب': "ب. نحدد الوقت والمكان، ونترك بقية التفاصيل لوقتها",
    'ج': "ج. نتواصل في نفس اليوم ونقرر حسب المزاج والظروف",
  },
  5: {
    'أ': "أ. طلعات ونشاطات فيها مجموعة من الناس",
    'ب': "ب. جلسة هادئة مع شخص أو شخصين",
    'ج': "ج. وقت فردي وهدوء قبل بداية الأسبوع الجديد",
  },
}
...
const EARLY_OPENNESS_LABELS: Record<number, string> = {
  0: "أ. أبقى غالبًا في الاهتمامات والأحداث والمواضيع العامة",
  1: "ب. أشارك آراء وتجارب خفيفة، بدون تفاصيل شخصية كثيرة",
  2: "ج. ممكن أشارك موضوعًا شخصيًا واحدًا إذا جاء في سياق السالفة",
  3: "د. أرتاح من البداية لمشاركة قصص وتجارب شخصية",
}
...
const ATTACHMENT_QUESTIONS: Record<string, { label: string, options: Record<string, string> }> = {
  attachment_1: { label: "بعد عدة أيام بدون تواصل:", options: { 'أ': 'أفترض أنه مشغول', 'ب': 'أتساءل إذا تغيّر شيء', 'ج': 'لا أبادر غالبًا', 'د': 'أرغب بالتواصل لكن أتردد' } },
  attachment_2: { label: "عند حدوث خلاف:", options: { 'أ': 'أواجه بهدوء', 'ب': 'أتجنب المواجهة', 'ج': 'أنسحب', 'د': 'أقترب ثم أبتعد' } },
  attachment_3: { label: "لما تصير العلاقة أقرب:", options: { 'أ': 'أشارك أكثر بالتدريج', 'ب': 'أحتاج وضوح الاهتمام', 'ج': 'أحتاج حدودًا ومساحة', 'د': 'أرغب بالقرب مع حذر' } },
  attachment_4: { label: "وقت الضغط أو المشكلة:", options: { 'أ': 'أشرح لشخص أثق فيه', 'ب': 'أتواصل أكثر وأحتاج ردًا سريعًا', 'ج': 'أرتب الموضوع بنفسي أولًا', 'د': 'أطلب الدعم ثم أبتعد قليلًا' } },
...
const COMMUNICATION_QUESTIONS: Record<string, { label: string, options: Record<string, string> }> = {
  communication_1: { label: "بعد تجاوز شخص لحدودك:", options: { 'أ': 'أقول مباشرة وش أزعجني', 'ب': 'ما أفتح الموضوع', 'ج': 'أعترض بنبرة قوية', 'د': 'يتغيّر تعاملي بدون كلام مباشر' } },
  communication_2: { label: "عند احتياج شيء مهم:", options: { 'أ': 'أقول طلبي بوضوح', 'ب': 'أنتظر أن يلاحظ', 'ج': 'أكرر أو أشدد على طلبي', 'د': 'أقول عادي ويظهر انزعاجي' } },
  communication_3: { label: "عند اختلاف رأي جماعي:", options: { 'أ': 'أشرح اختلافي', 'ب': 'أمشي مع رأي المجموعة', 'ج': 'أرد بقوة على نقاط الضعف', 'د': 'يظهر اعتراضي لاحقًا بشكل غير مباشر' } },
  communication_4: { label: "لما يرتفع توترك:", options: { 'أ': 'أشرح توتري', 'ب': 'أسكت أو أنسحب', 'ج': 'يصير كلامي أحدّ', 'د': 'يقل تواصلي أو يبرد' } },
  communication_5: { label: "عند اختلاف مهم مع شخص قريب:", options: { 'أ': 'أوضح موقفي وأفهمه', 'ب': 'أسكت حتى لا تصبح مشكلة', 'ج': 'أتمسك وأحاول إثبات رأيي', 'د': 'أوصل اعتراضي بالمزح أو التلميح' } },
}

const CORE_VALUES_QUESTIONS: Record<string, { label: string, options: Record<string, string> }> = {
  core_values_1: { label: "عند سؤال المدير عن خطأ صديق:", options: { 'أ': 'أقول اللي أعرفه', 'ب': 'أترك له شرح التفاصيل', 'ج': 'أحافظ على خصوصيته' } },
  core_values_2: { label: "ترك الوظيفة لمشروع مجرّب:", options: { 'أ': 'يبدأ بدوام كامل', 'ب': 'ينتقل تدريجيًا', 'ج': 'ينتظر دخلًا ثابتًا' } },
  core_values_3: { label: "الاختلاف في القيم الأساسية:", options: { 'أ': 'جزء طبيعي من العلاقة', 'ب': 'يحتاج حدودًا واضحة', 'ج': 'أفضل التقارب في الأساسيات' } },
  core_values_4: { label: "وقت صعب مع مساحة من القريب:", options: { 'أ': 'المساحة تناسبني', 'ب': 'أفضل رسالة أحيانًا', 'ج': 'أحتاج حضورًا متكررًا' } },
  core_values_5: { label: "خلاف صديق مع شخص تعرفه:", options: { 'أ': 'أفصل بين العلاقتين', 'ب': 'أفهم وأقلل مؤقتًا', 'ج': 'أنهي علاقتي بالشخص الآخر' } },
}
