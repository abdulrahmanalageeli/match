...
export default function PromptTopicsModal({ open, onClose, dark }: { open: boolean; onClose: () => void; dark: boolean }) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogPortal>
        <DialogOverlay className={`fixed inset-0 z-50 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 ${dark ? 'bg-black/80' : 'bg-black/20'}`} />
        <DialogContent
