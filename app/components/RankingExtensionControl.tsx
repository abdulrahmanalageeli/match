...
export default function RankingExtensionControl({ name, extension, disabled, onGrant, oneMinuteOnly = false }: {
  name: string
  extension?: { expires_at: string } | null
  disabled?: boolean
  oneMinuteOnly?: boolean
  ...
  return <div className="mt-3 space-y-2 rounded-xl border border-cyan-300/20 bg-cyan-300/[0.04] p-3" dir="rtl">
    ...
    <div className="flex flex-wrap gap-2">
      {oneMinuteOnly ? <span className="flex min-h-11 items-center text-xs text-cyan-100">دقيقة واحدة فقط</span> : <select aria-label={`مدة إضافية لـ ${name}`} value={seconds} disabled={disabled || busy} onChange={e => setSeconds(Number(e.target.value))} className="min-h-11 rounded-lg border border-white/10 bg-slate-900 px-2 text-xs text-white">
      ...
      <button type="button" disabled={disabled || busy} onClick={async () => {
        ...
        try { await onGrant(oneMinuteOnly ? 60 : seconds) } catch (err) { setError(err instanceof Error ? err.message : "تعذّر منح الوقت الإضافي") }
