...
export function RoomRoundTimer({ event, clockOffsetMs, disabled = false, stale = false, projector = false, onCommand }: Props) {
  ...
  const status = finished ? "خلص وقت الجولة" : running ? "المؤقت شغّال" : paused ? "المؤقت متوقف" : "جاهز"
  ...
  return (
    <section data-room-timer aria-label={`وقت الجولة ${event.active_round}`} className={`${projector ? "border-y border-white/10 bg-white/[0.025] px-4 py-3" : "rounded-[1.5rem] border border-[#d7ba7d]/20 bg-[#211f1a] p-4 sm:p-5"} text-white`}>
      <div className={`flex ${projector ? "flex-wrap justify-center gap-x-8 gap-y-1" : "justify-between gap-4"} items-center`}>
        <div>
          <p className="flex items-center gap-2 text-xs font-bold text-[#d6b77f]"><Clock3 size={16} /> وقت الجولة {event.active_round}</p>
      ...
      {stale && <p role="status" className="mt-2 text-center text-xs font-bold text-amber-200">الاتصال انقطع. الوقت حسب آخر تحديث.</p>}
      {onCommand && <>
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button type="button" disabled={controlsDisabled || finished} onClick={() => onCommand(running ? "pause" : "start")} className="flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-[#e1bd7c] px-3 text-sm font-extrabold text-[#211f1a] disabled:opacity-35">
            {running ? <Pause size={17} /> : <Play size={17} />}{running ? "إيقاف مؤقت" : paused ? "كمّل المؤقت" : "شغّل المؤقت"}
          </button>
          <button type="button" disabled={controlsDisabled || (!event.timer_ends_at && remaining === event.timer_duration_seconds)} onClick={() => onCommand("reset")} className="flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-white/15 px-3 text-sm font-extrabold text-white/80 disabled:opacity-35"><RotateCcw size={17} /> إعادة الوقت</button>
        </div>
        <details className="mt-3 border-t border-white/10 pt-1">
          <summary className="min-h-11 cursor-pointer py-3 text-xs font-bold text-white/65">المدة: {event.timer_duration_seconds / 60} دقيقة · {running ? "وقّف المؤقت عشان تعدّلها" : "تغيير المدة"}</summary>
          ...
          <label className="flex items-center gap-2 text-xs font-bold text-white/65">مدة الجولة (دقيقة)
          ...
          <button type="submit" disabled={controlsDisabled || running || !validMinutes || !durationChanged} className="min-h-11 rounded-xl border border-white/15 px-4 text-xs font-bold text-[#efd6a6] disabled:opacity-30">حفظ المدة</button>
          </form>
          <p className="mt-2 text-[11px] leading-5 text-white/55">من دقيقة إلى 120 دقيقة. تغيير المدة يرجّع الوقت من البداية.</p>
        </details>
        <p className="text-[11px] leading-5 text-white/55">إذا خلص الوقت، اضغط «الجولة التالية».</p>
      </>}
    </section>
  )
}
