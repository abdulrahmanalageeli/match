  ...
  return <section dir="rtl" className="space-y-4">
    ...
    {tab === "participants" && <div className="space-y-3">
      <div className="sticky top-2 z-20 grid gap-2 rounded-2xl border border-white/10 bg-slate-950/90 p-3 shadow-xl backdrop-blur-xl sm:grid-cols-[1fr_220px]">
        ...
        <select value={participantFilter} onChange={e => setParticipantFilter(e.target.value)} className={fieldClass}><option value="all">كل المشاركين</option><option value="incomplete_questions">لم يكملوا الأسئلة الجديدة</option><option value="complete_questions">أكملوا الأسئلة الجديدة</option><option value="confirmed">مؤكد</option><option value="awaiting_payment">بانتظار الدفع</option><option value="declined">معتذر</option><option value="on_way">في الطريق</option><option value="late">متأخر</option><option value="arrived">وصل</option></select>
      </div>
      ...
      <div className="grid gap-3 xl:grid-cols-2">{participants.map((p: any) => <details key={p.id} className="group rounded-2xl border border-white/10 bg-white/[0.04] open:border-cyan-400/20 open:bg-cyan-400/[0.035]">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4"><div className="flex min-w-0 items-start gap-3"><input type="checkbox" checked={selectedNumbers.has(p.assigned_number)} onClick={e => e.stopPropagation()} onChange={e => setSelectedNumbers(current => { const next = new Set(current); e.target.checked ? next.add(p.assigned_number) : next.delete(p.assigned_number); return next })} className="mt-1 h-4 w-4 shrink-0 accent-cyan-400" /><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-lg bg-white/10 px-2 py-1 text-xs font-black">#{p.assigned_number}</span><h4 className="truncate font-black">{participantName(p)}</h4>{p.attendance_confirmed ? <span className="text-[10px] font-bold text-emerald-300">مؤكد</span> : p.attendance_denied_at ? <span className="text-[10px] font-bold text-red-300">معتذر</span> : <span className="text-[10px] font-bold text-slate-500">لم يرد</span>}</div><div className="mt-2 flex flex-wrap gap-1.5"><span className={`rounded-full px-2 py-0.5 text-[10px] ${p.PAID_DONE || p.payment_waived ? "bg-emerald-500/15 text-emerald-300" : "bg-amber-500/15 text-amber-300"}`}>{p.payment_waived ? "معفى" : p.PAID_DONE ? "مدفوع" : "غير مدفوع"}</span>{p.arrival_status && <span className="rounded-full bg-orange-500/15 px-2 py-0.5 text-[10px] text-orange-300">{p.arrival_status}</span>}{p.age_flex_years > 0 && <span className="rounded-full bg-violet-500/15 px-2 py-0.5 text-[10px] text-violet-300">مرونة عمر ±{p.age_flex_years}</span>}{(() => { const completion = getParticipantMatchInsightsCompletion(p); return <span className={`rounded-full px-2 py-0.5 text-[10px] ${completion.complete ? "bg-emerald-500/15 text-emerald-300" : "bg-teal-500/15 text-teal-200"}`}>{completion.complete ? "الأسئلة الجديدة ✓" : `الأسئلة الجديدة ${completion.completed}/${completion.total}`}</span> })()}</div></div></div><ChevronDown className="h-5 w-5 shrink-0 text-slate-500 transition group-open:rotate-180" /></summary>
