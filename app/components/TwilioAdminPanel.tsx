...
function ApprovalsPanel({ data, adminPassword, onRefresh }: { data: any; adminPassword: string; onRefresh: () => void }) {
  ...
  return <div className="grid gap-4 xl:grid-cols-2">
    <section className="rounded-2xl border border-cyan-400/15 bg-cyan-400/[0.035] p-4"><div className="mb-4 flex items-center justify-between"><div><h3 className="font-black">مراجعة الإيصالات</h3><p className="text-xs text-slate-500">{receipts.length} بانتظار القرار</p></div><FileCheck2 className="h-5 w-5 text-cyan-300" /></div><div className="space-y-2">{receipts.map((p: any) => <div key={p.id} className="rounded-xl border border-white/10 bg-slate-950/50 p-3"><div className="flex items-start justify-between gap-2"><div><p className="text-sm font-black">#{p.assigned_number} · {participantName(p)}</p><p className="mt-1 text-[10px] text-slate-500">{fmtDate(p.receipt_received_at)}</p></div><a href={p.receipt_url} target="_blank" rel="noreferrer" className="text-xs font-bold text-cyan-300">عرض الإيصال</a></div><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => adminCall("approve-receipt", { assigned_number: p.assigned_number })} disabled={Boolean(working)} className={`${buttonClass} border-emerald-400/20 bg-emerald-400/10 text-emerald-200`}><Check className="h-4 w-4" />اعتماد</button><button onClick={() => { const reason = window.prompt("سبب الرفض (اختياري)") ?? ""; adminCall("reject-receipt", { assigned_number: p.assigned_number, reason }) }} disabled={Boolean(working)} className={`${buttonClass} border-red-400/20 bg-red-400/10 text-red-200`}><XCircle className="h-4 w-4" />رفض</button></div></div>)}{receipts.length === 0 && <EmptyState text="لا توجد إيصالات معلقة" />}</div></section>
    <section className="rounded-2xl border border-amber-400/15 bg-amber-400/[0.035] p-4"><div className="mb-4 flex items-center justify-between"><div><h3 className="font-black">طلبات الحضور</h3><p className="text-xs text-slate-500">{data.attendanceRequests?.length || 0} طلبات معلقة</p></div><UserCheck className="h-5 w-5 text-amber-300" /></div><div className="space-y-2">{(data.attendanceRequests || []).map((r: any) => <div key={r.id} className="rounded-xl border border-white/10 bg-slate-950/50 p-3"><div className="flex items-center justify-between"><div><p className="text-sm font-black">#{r.assigned_number}</p><p className="mt-1 text-[10px] text-slate-500">{r.request_type === "confirm" ? "طلب تأكيد" : "اعتذار"} · {fmtDate(r.created_at)}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${r.request_type === "confirm" ? "bg-emerald-500/15 text-emerald-300" : "bg-red-500/15 text-red-300"}`}>{r.request_type}</span></div><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => adminCall("approve-attendance-request", { id: r.id })} disabled={Boolean(working)} className={`${buttonClass} border-emerald-400/20 bg-emerald-400/10 text-emerald-200`}>إغلاق كمراجع</button><button onClick={() => adminCall("reject-attendance-request", { id: r.id })} disabled={Boolean(working)} className={`${buttonClass} border-red-400/20 bg-red-400/10 text-red-200`}>رفض الطلب</button></div></div>)}{!data.attendanceRequests?.length && <EmptyState text="لا توجد طلبات حضور معلقة" />}</div></section>
  </div>
}
