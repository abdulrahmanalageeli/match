...
import { ArrowLeft, BookOpen, Check, CheckCircle, Flag, HelpCircle, Lock, Users, X } from "lucide-react";
...
export default function SocialVoteActivity({ activityId, round = 1, participantNames = EMPTY_NAMES, rulebook, onRulebookChange, onSharedContentChange, onFinish }: {
  ...
  const [stage, setStage] = useState<"setup" | "handoff" | "vote" | "reveal" | "complete">("setup");
  ...
  return (
    <section dir="rtl" aria-label={title} className="mx-auto w-full max-w-md space-y-4 pb-4 text-right">
      ...
      {stage !== "setup" && stage !== "complete" && <>
        <div className="flex items-center justify-between text-xs font-bold text-white/55"><span>الموقف {numberAr(promptIndex + 1)} من {numberAr(prompts.length)}</span><span>{stage === "reveal" ? "انكشفت الآراء" : "التصويت مخفي"}</span></div>
      ...
      <div className="activity-panel space-y-4 p-5 sm:p-6">
        <div className="flex items-center gap-2 text-[11px] font-bold text-teal-200">{isRules ? <BookOpen size={16} /> : <Flag size={16} />}{stage === "complete" ? "وش أخذنا من السالفة؟" : stage === "setup" ? "هاتف واحد · كل صوت له مساحة" : promptTitle}</div>
        ...
        {stage === "setup" ? <>
          <p className="text-sm leading-7 text-white/75">مرّروا الهاتف بالدور. كل شخص يختار رأيه بسرّية، وبعد آخر تصويت تنكشف الأعداد بدون أسماء. اسمعوا الأسباب، وعادي تتجاوزون أي سؤال.</p>
          ...
          {names.length ? <p className="flex items-center gap-2 text-sm text-teal-100"><Users size={17} /> {numberAr(names.length)} مشاركين · الأسماء تحدد الدور فقط</p> : <label className="flex items-center justify-between gap-3 text-sm font-bold text-white/80">عدد المشاركين<select value={playerCount} onChange={event => setPlayerCount(Number(event.target.value))} className="min-h-11 rounded-xl border border-white/15 bg-gray-950 px-4 text-white focus-visible:ring-2 focus-visible:ring-teal-200">{Array.from({ length: 11 }, (_, index) => index + 2).map(count => <option key={count} value={count}>{numberAr(count)}</option>)}</select></label>}
          <p className="text-xs leading-6 text-white/45">٣٠ موقف · جرّبوا ٣–٥ مواقف في حوالي ٨ دقائق، وكمّلوا على راحتكم.</p>
          <button type="button" className="event3-art-action activity-primary w-full" onClick={() => setStage("handoff")}>نبدأ التصويت <ArrowLeft size={17} /></button>
        </> : stage === "handoff" ? <>
          ...
          <div className="space-y-3" aria-label="نتائج التصويت">{results.map(({ choice: label, count }) => <div key={label}><div className="mb-2 flex items-center justify-between text-sm font-bold text-white/85"><span>{label}</span><span>{numberAr(count)} من {numberAr(playerCount)}</span></div><div className="h-2 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-gradient-to-l from-violet-400 via-cyan-400 to-teal-300" style={{ width: `${count / playerCount * 100}%` }} /></div></div>)}</div>
          {skipped > 0 && <p className="text-xs text-white/50">تجاوزوا السؤال: {numberAr(skipped)} · ما ينحسب كتصويت</p>}
          <div className="rounded-2xl border border-teal-300/15 bg-teal-300/5 p-4"><p className="mb-2 text-[11px] font-bold text-teal-200">نسمع السبب، بدون ما نخمن مين صوّت</p><p className="text-base font-bold leading-7 text-white">{discussion}</p><p className="mt-2 text-xs leading-6 text-white/55">{isRules ? "هل تصلح قاعدة لكم؟ اسمعوا المختلف، واتفقوا قبل إضافتها." : "اللي اختار «يعتمد»: وش التفصيلة اللي ممكن تغيّر رأيك؟ المشاركة على راحتكم."}</p></div>
