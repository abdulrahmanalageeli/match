...
test("ranking bootstrap distinguishes dependency failures without changing saved-order semantics", async () => {
  ...
  assert.match(rankingBootstrap, /draft_order: extension\?\.ranked_numbers \|\| \(pendingDraft \? draft\.ranked_numbers : null\)/)
  assert.match(rankingBootstrap, /already_submitted: !extension && !pendingDraft && nums\.every/)
