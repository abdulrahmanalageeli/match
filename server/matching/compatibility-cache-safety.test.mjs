...
test('manual pair test mode calculates fresh without reading or writing compatibility cache', async () => {
  const [source, adminSource] = await Promise.all([
    read('api/admin/trigger-match.mjs'),
    read('app/routes/admin.tsx'),
  ])
  const block = between(source, '// Handle manual match creation', '// Note: Payment filtering is NOT applied')
  const calculation = between(block, '// Manual pair test/analyze mode', 'const mbtiScore')

  assert.match(calculation, /const freshManualTest = manualMatch\.testModeOnly === true/)
  assert.match(calculation, /freshManualTest \? false : skipAI/)
  assert.match(calculation, /freshManualTest \? \{ skipCacheLookup: true, skipCacheWrite: true \} : \{\}/)
  assert.match(block, /cache_status: freshManualTest \? 'bypassed'/)
  assert.match(block, /calculation_source: freshManualTest \? 'fresh'/)
  assert.match(adminSource, /Test mode · Fresh · Read only/)
...
test('cache storage bulk-upserts arrays in bounded chunks', async () => {
  ...
  assert.match(bulkStore, /storableChunk\.map\(item => item\.row\)/)
...
test('required AI chemistry uses a protected durable queue with atomic 12-job claims', async () => {
  const [source, migration, schedulerMigration, worker, vercel] = await Promise.all([
    read('api/admin/trigger-match.mjs'),
  ...
  assert.match(atomicStore, /\.rpc\('store_deferred_v11_compatibility_cache'/)
  assert.match(atomicStore, /p_cache_rows: items\.map\(item => item\.row\)/)
  assert.match(atomicStore, /p_jobs: jobs/)
  assert.match(bulkStore, /storeDeferredVibeRowsAndJobs\(deferredItems/)
  assert.match(bulkStore, /chunkSize = 250/)
  assert.match(migration, /create or replace function public\.store_deferred_v11_compatibility_cache/i)
  assert.match(migration, /jsonb_populate_recordset/i)
  assert.match(migration, /do not allow a delayed foreground request to downgrade it/i)
  assert.match(migration, /for update skip locked/i)
  assert.match(migration, /limit greatest\(1, least\(coalesce\(p_limit, 12\), 12\)\)/i)
  assert.match(migration, /where status = 'pending'/i)
  assert.match(migration, /enable row level security/i)
  assert.match(migration, /revoke all on table public\.compatibility_vibe_enrichment_jobs from authenticated/i)
...
test('batched cache keeps cheap scans separate and delta reserves independent local and AI lanes', async () => {
  ...
  assert.match(full, /const effectiveMaxAICaches[\s\S]*12[\s\S]*12/)
  assert.match(full, /const effectiveMaxLocalCaches[\s\S]*500[\s\S]*500/)
  assert.match(full, /targetJobs\.length >= targetLimit/)
  assert.match(full, /pairs_processed: pairsScanned/)
  assert.match(full, /cache_jobs_started: cacheJobsStarted/)
  assert.match(full, /skipCacheLookup: true, skipCacheWrite: true/)
  assert.match(full, /const progressiveLocalWaveSize = 250/)
  assert.match(full, /localJobs\.slice\(start, start \+ progressiveLocalWaveSize\)\.map\(executeFullCacheJob\)/)
  assert.match(full, /aiJobs\.slice\(start, start \+ effectiveMaxAICaches\)\.map\(executeFullCacheJob\)/)
  assert.match(full, /storeCachedCompatibilities\([\s\S]*durableJobs\.map\(job => job\.entry\)[\s\S]*chunkSize: 250/)
  assert.match(source, /cacheUpsertRetry\('Compatibility cache bulk upsert'/)
  ...
  assert.match(delta, /const effectiveMaxAICaches[\s\S]*12[\s\S]*12/)
  assert.match(delta, /const effectiveMaxLocalCaches[\s\S]*500[\s\S]*500/)
  assert.match(delta, /const progressiveLocalWaveSize = 250/)
  assert.match(delta, /localJobs\.slice\(start, start \+ progressiveLocalWaveSize\)\.map\(executeJob\)/)
  assert.match(delta, /aiJobs\.slice\(start, start \+ effectiveMaxAICaches\)\.map\(executeJob\)/)
  assert.match(delta, /await storeCompletedDeltaWave\(localWaveResults\)/)
  assert.match(delta, /await storeCompletedDeltaWave\(aiWaveResults\)/)
  assert.match(delta, /storeCachedCompatibilities\(cacheEntries, \{[\s\S]*chunkSize: 250[\s\S]*queueDeferredAI: true/)
  assert.ok(delta.indexOf('await storeCompletedDeltaWave(localWaveResults)') < delta.indexOf('const aiWaveResults'))
  ...
  assert.match(modalSource, /const MAX_BATCH_ATTEMPTS = 5/)
  assert.match(modalSource, /const MAX_AI_CACHES_PER_REQUEST = 12/)
  assert.match(modalSource, /const MAX_LOCAL_CACHES_PER_REQUEST = 500/)
  assert.match(modalSource, /deferAIEnrichment: true/)
  assert.match(modalSource, /batchStart: requestStart[\s\S]*resumeCursor: requestResumeCursor/)
  assert.match(modalSource, /attemptErrors === 0 \|\| attempt === MAX_BATCH_ATTEMPTS/)
  assert.match(modalSource, /Final check found \$\{missingCoverage\} missing cache pair/)
  assert.match(modalSource, /readFullCacheCheckpoint\(eventId, mode\)/)
  assert.match(modalSource, /writeFullCacheCheckpoint\(/)
  assert.match(modalSource, /let lastAcknowledgedProgress = savedCheckpoint\?\.progress \|\| null/)
  assert.match(modalSource, /nextStart: requestStart,[\s\S]*resumeCursor: requestResumeCursor,[\s\S]*progress: lastAcknowledgedProgress/)
  assert.match(modalSource, /checkpointId,/)
  assert.match(modalSource, /CACHE_CHECKPOINT_STALE/)
  assert.match(modalSource, /label="Pairs Scanned"/)
  assert.match(adminSource, /let aiCachesPerRequest = savedCheckpoint\?\.aiCachesPerRequest \|\| 12/)
  assert.match(adminSource, /action: "delta-pre-cache-batched"[\s\S]*skipAI: false[\s\S]*deferAIEnrichment: true[\s\S]*maxAICachesPerRequest: aiCachesPerRequest[\s\S]*maxLocalCachesPerRequest: 500[\s\S]*maxPairsPerRequest: 20000[\s\S]*checkpointId/)
  assert.match(adminSource, /readDeltaCacheCheckpoint\(currentEventId\)/)
  assert.match(adminSource, /persistDeltaCheckpoint\(currentProgress, resumeCursor, coverageRepairPairs\)/)
  assert.match(adminSource, /CACHE_CHECKPOINT_STALE/)
  assert.match(source, /function buildCacheRunCheckpointId/)
  assert.match(full, /kind: `full:\$\{genderMode\}`/)
  assert.match(delta, /kind: 'delta'/)
  assert.doesNotMatch(adminSource, /AI batch timed out/)
