...
test('batched cache keeps cheap scans separate and delta reserves independent local and AI lanes', async () => {
  ...
  assert.match(full, /const maxConcurrentCacheJobs = 8/)
  assert.match(full, /skipCacheLookup: true, skipCacheWrite: true/)
  assert.match(full, /storeCachedCompatibilities\([\s\S]*durableJobs\.map\(job => job\.entry\)/)
  ...
  assert.match(delta, /const progressiveLocalWaveSize = 100/)
  ...
  assert.match(delta, /storeCachedCompatibilities\(cacheEntries, \{ chunkSize: 100 \}\)/)
  ...
  assert.match(modalSource, /const MAX_AI_CACHES_PER_REQUEST = 8/)
  ...
  assert.match(adminSource, /action: "delta-pre-cache-batched"[\s\S]*skipAI: false[\s\S]*maxAICachesPerRequest: aiCachesPerRequest[\s\S]*maxLocalCachesPerRequest: 500[\s\S]*maxPairsPerRequest: 20000[\s\S]*checkpointId/)
